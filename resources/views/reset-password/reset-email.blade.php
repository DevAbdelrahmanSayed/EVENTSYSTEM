    ...
    >
        ...
        <div class="flex flex-col justify-center p-8 md:p-14">
            ...
            <form method="post" action="{{route('reset-email')}}">
                @csrf
                <div class="py-4">
                    <label for="email" class="block mb-2 text-sm font-medium text-[#FFFFFF] dark:text-white">Enter your
                        email address</label>
                    <input type="email" name="email" id="email" placeholder="example@uskudar.edu.tr"
            ...
            <a href="{{route('login')}}"
               class="text-sm font-medium text-[#FFFFFF] hover:underline dark:text-white text-center mt-2">Back to
                login</a>
        </div>
        <!-- {/* right side */} -->
